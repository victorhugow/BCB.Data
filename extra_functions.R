...
#Pega os Scores de Crédito do Santander no Outlook
get_score_s3 = function(){
  require(tidyverse)
  require(Microsoft365R)
  outl <- get_business_outlook()
  inbox = outl$get_inbox()
  curvas_marcacao = inbox$list_emails(search =  'Curvas Marcação - S3 Caceis', n = 999)
  
  curvas_marcacao = lapply(curvas_marcacao, function(x) x$list_attachments()) %>% unlist()
  planilhas_curvas = curvas_marcacao[grep(lapply(curvas_marcacao, function(x) x$properties$name) %>% unlist(),pattern = 'Matriz')]
  
  df_scores_s3 = map_df(planilhas_curvas, 
                        function(x){
                          tp_dir = tempdir()
                          tp_file = tempfile(tmpdir = tp_dir,fileext = '.xlsx')
                          x$download(dest = tp_file, overwrite = T)
                          data = x$properties$name %>% str_extract('\\d+')
                          score_s3 = readxl::read_excel(tp_file, range = 'A1:C1000') %>% na.omit() %>% mutate(data = as.Date(data, '%d%m%Y'))
                          colnames(score_s3) <- c('banco', 'ymf', 'rating', 'data')
                          score_s3
                        })
